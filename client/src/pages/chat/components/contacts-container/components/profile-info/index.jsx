...
const ProfileInfo = () => {
  ...
  const logOut = async () => {
    try {
        closeChat();   // this will logout chat info if it is not closed by cross button and directly logout button is hit
  ...
  return (
    <div className="absolute bottom-0 h-16 flex items-center justify-between px-10 w-full bg-[#2a2b33]">
            ...
            <Avatar className="h-12 w-12 rounded-full overflow-hidden">
              {
                ...
                // we can also use charAt(0) instead of split("").shift() to get the first character
              }
            </Avatar>
        </div>
        <div>
      ...
      <div className="flex gap-5">
        <Tooltip>
            <TooltipTrigger><FiEdit2 className="text-purple-500 tex-xl font-medium" onClick={() => {navigate('/profile')}}/></TooltipTrigger>
            <TooltipContent className="bg-[#1c1b1e] border-none text-white">Edit Profile</TooltipContent>
        </Tooltip>

        <Tooltip>
            <TooltipTrigger><IoPowerSharp className="text-red-500 tex-xl font-medium" onClick={logOut}/></TooltipTrigger>
